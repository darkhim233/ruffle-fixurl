...
pub fn load_data_into_url_loader<'gc>(
    ...
    // 如果 URL 以 "file://" 开头且不是 "file:///"（即本地文件），则强转成 https://
    let fixed_url = if original_url.starts_with("file://") && !original_url.starts_with("file:///") {
        original_url.replace("file://", "https://")
    } else {
        original_url
    };
    // 重新构造 Request（仅 GET，因为 URLLoader 通常用于 GET）
